...
class IncrementalSamples4grid(IncrementalPersonReIDSamples):
    """GRID.
    ...
    def __init__(self, datasets_root, relabel=True, combineall=False, split_id=0):
        ...
        # new_train=(tuple(item) for item in train)
        # query = (tuple(item + ['grid'] + [item[1]]) for item in query)
        # gallery = (tuple(item + ['grid'] + [item[1]]) for item in gallery)
        # train = [tuple(item + [17] ) for item in train]
        # query = [tuple(item + [17] ) for item in query]
        # gallery = [tuple(item + [17] ) for item in gallery]
            ...
            for split_idx in range(10):
                ...
                for img_path in probe_img_paths:
                    ...
                    if img_idx in train_idxs:
                        train.append((img_path, idx2label[img_idx], camid,12))
                    else:
                        query.append((img_path, img_idx, camid,12))
                ...
                for img_path in gallery_img_paths:
                    ...
                    if img_idx in train_idxs:
                        ...
                        gallery.append((img_path, img_idx, camid,12))
