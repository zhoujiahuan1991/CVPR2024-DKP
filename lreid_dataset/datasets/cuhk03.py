...
class IncrementalSamples4cuhk03(IncrementalPersonReIDSamples):
    '''
     Dataset statistics:
        - identities: 1360.
        - images: 13164.
        ...
        - splits: 20 (classic).
    '''
    dataset_dir = 'CUHK03_lifelong'
    def __init__(self, datasets_root, relabel=True, combineall=False, split_id=0, cuhk03_labeled=True,
        ...
        # train = [tuple(item + ['cuhk03'] + [item[1]]) for item in split['train']]
        # query = [tuple(item + ['cuhk03'] + [item[1]]) for item in split['query']]
        # gallery = [tuple(item + ['cuhk03'] + [item[1]]) for item in split['gallery']]
        train = [tuple(item + [4]) for item in split['train']]
        query = [tuple(item + [4] ) for item in split['query']]
        gallery = [tuple(item + [4] ) for item in split['gallery']]
